...
import engineer2 from '../../assets/images/engineer-2.png'
import workshop from '../../assets/images/workshop.png'
import ezscan from '../../assets/images/ezscan.png'
  ...
  return (
    <>
      <div className="bc"><div className="bc-in">
        ...
        <span>{t(lang, '연구개발 (R&D)', 'R&D')}</span>
      </div></div>

      {/* 히어로 이미지 */}
      <div style={{ position: 'relative', height: 320, overflow: 'hidden' }}>
        <img src={engineer2} alt="R&D" style={{ width: '100%', height: '100%', objectFit: 'cover', objectPosition: 'center 30%' }} />
        ...
        <div style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', textAlign: 'center' }}>
          ...
          <h1 style={{ fontFamily: "'Rajdhani', sans-serif", fontSize: 'clamp(28px,4.5vw,54px)', fontWeight: 700, color: '#fff', letterSpacing: 2, textTransform: 'uppercase', marginTop: 8 }}>
            {t(lang, '연구개발 R&D', 'Research & Development')}
          </h1>
          <p style={{ fontSize: 14, color: 'rgba(255,255,255,0.65)', marginTop: 10 }}>
            {t(lang, 'One-Stop R&D — 생산관리부터 서보 제어까지', 'One-Stop R&D — from production management to servo control')}
          </p>
        </div>
      </div>
      ...
      <div className="outer"><div className="sec">
        <div className="vis-grid">
          {rndItems.map(({ num, titleKo, titleEn, feats, badges }) => (
            <div className="vis-card" key={num}>
              ...
              <h3>{t(lang, titleKo, titleEn)}</h3>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8, marginTop: 8 }}>
                {feats.map(({ ko, en }) => (
                  <div className="prod-feat" key={ko}>{t(lang, ko, en)}</div>
                ))}
              </div>
        ...
        <div style={{ marginTop: 70 }}>
          <div className="eyebrow">{t(lang, '작업 환경', 'Work Environment')}</div>
          <h2 className="stitle"><span>{t(lang, '작업장 ', 'Our ')}</span><em>{t(lang, '환경', 'Workplace')}</em></h2>
          <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr 1fr', gridTemplateRows: '220px 220px', gap: 2, background: 'var(--border)', marginTop: 36 }}>
            <div style={{ gridRow: 'span 2', overflow: 'hidden' }}>
              <img src={workshop} alt="Workshop" style={{ width: '100%', height: '100%', objectFit: 'cover', transition: 'transform 0.4s' }}
                onMouseOver={e => e.target.style.transform = 'scale(1.03)'}
                onMouseOut={e => e.target.style.transform = 'scale(1)'} />
            </div>
            <div style={{ overflow: 'hidden' }}>
              <img src={engineer2} alt="Engineer" style={{ width: '100%', height: '100%', objectFit: 'cover', objectPosition: 'center top', transition: 'transform 0.4s' }}
                onMouseOver={e => e.target.style.transform = 'scale(1.04)'}
                onMouseOut={e => e.target.style.transform = 'scale(1)'} />
            </div>
            <div style={{ overflow: 'hidden', background: 'var(--card)' }}>
              <img src={ezscan} alt="EZScan" style={{ width: '100%', height: '100%', objectFit: 'contain', padding: 16, transition: 'transform 0.4s' }}
                onMouseOver={e => e.target.style.transform = 'scale(1.04)'}
                onMouseOut={e => e.target.style.transform = 'scale(1)'} />
            </div>
            <div style={{ overflow: 'hidden' }}>
              <img src={office} alt="Office" style={{ width: '100%', height: '100%', objectFit: 'cover', transition: 'transform 0.4s' }}
                onMouseOver={e => e.target.style.transform = 'scale(1.04)'}
                onMouseOut={e => e.target.style.transform = 'scale(1)'} />
            </div>
            <div style={{ overflow: 'hidden' }}>

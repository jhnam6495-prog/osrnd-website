  ...
  return (
    <div style={{ background: 'var(--card)', padding: 40, position: 'relative', overflow: 'hidden' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', gap: 28 }}>
        <div style={{ flexShrink: 0, width: 180, border: '1px solid var(--border)', background: 'var(--panel)', overflow: 'hidden' }}>
  ...
  return (
    <>
      <div className="bc"><div className="bc-in">
        ...
        <span>{t(lang, '인증현황', 'Certifications')}</span>
      </div></div>
      <div className="ph"><div className="ph-in">
        <h1>{t(lang, '인증현황', 'Certifications')}</h1>
        <p>{t(lang, '오에스알앤디㈜가 보유한 인증서 및 등록증입니다.', 'Certificates and registrations held by OSRnD Co., Ltd.')}</p>
      </div></div>

      <div className="outer"><div className="sec">
        <div className="eyebrow">{t(lang, '보유 인증 및 등록', 'Certificates & Registrations')}</div>
        <h2 className="stitle"><em>{t(lang, '인증', 'Certifications')}</em><span>{t(lang, '현황', ' & Registrations')}</span></h2>
        <p className="sdesc">{t(lang, '오에스알앤디㈜는 공식 인증과 등록을 통해 신뢰성과 품질을 보장합니다.', 'OSRnD Co., Ltd. guarantees reliability and quality through official certifications and registrations.')}</p>

        {/* 1행: 사업자등록증 + R&D 인증서 */}
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 2, background: 'var(--border)', marginBottom: 2 }}>
          <CertCard
            accentColor="var(--cyan)"
            badge={t(lang, '등록증', 'Registration')}
            title={t(lang, '사업자등록증', 'Business Registration')}
            imgKo={certBizKo} imgEn={certBizEn} lang={lang}
            details={[
              [t(lang, '등록번호', 'Reg. No.'), '620-81-51336'],
              [t(lang, '법인명', 'Company'), t(lang, '오에스알앤디㈜', 'OSRnD Co., Ltd')],
              [t(lang, '대표자', 'CEO'), t(lang, '권오수', 'Kwon Ohsoo')],
              [t(lang, '개업일', 'Founded'), t(lang, '2014년 01월 01일', 'January 1, 2014')],
            ]}
            desc={t(lang,
          ...
          <CertCard
            accentColor="var(--cyan)"
            badge={t(lang, '인증서', 'Certificate')}
            title={t(lang, '연구개발전담부서 인증서', 'R&D Dept. Certification')}
            imgKo={certRndKo} imgEn={certRndEn} lang={lang}
            details={[
              [t(lang, '기관명', 'Company'), t(lang, '오에스알앤디㈜', 'OSRnD Co., Ltd')],
              [t(lang, '인증기관', 'Issuer'), t(lang, '과학기술정보통신부', 'Ministry of Science and ICT')],
              [t(lang, '인증분야', 'Field'), t(lang, '공장자동화 및 연구개발', 'Factory Automation & R&D')],
            ]}
            desc={t(lang,
        ...
        <div style={{ marginBottom: 2 }}>
          <div style={{ background: 'var(--panel)', border: '1px solid var(--border)', borderBottom: 'none', padding: '16px 28px', display: 'flex', alignItems: 'center', gap: 16 }}>
            ...
            <span style={{ fontFamily: "'Rajdhani', sans-serif", fontSize: 15, fontWeight: 700, color: 'var(--cyan)', letterSpacing: 2, textTransform: 'uppercase' }}>{t(lang, 'ISO 국제 인증', 'ISO International Certifications')}</span>
            <span style={{ fontSize: 11, color: 'var(--muted)', fontFamily: "'DM Mono', monospace" }}>{t(lang, '품질 · 환경 · 안전보건 경영시스템', 'Quality · Environmental · Occupational Health & Safety')}</span>
          </div>
            ...
            {[
              ...
              <div key={code} style={{ background: 'var(--card)', padding: '32px 28px', position: 'relative', overflow: 'hidden' }}>
                ...
                <div style={{ textAlign: 'center' }}>
                  ...
                  <div style={{ fontSize: 11, letterSpacing: 2, color: 'var(--muted)', fontFamily: "'DM Mono', monospace", textTransform: 'uppercase', marginBottom: 12 }}>{t(lang, labelKo, labelEn)}</div>
                  <p style={{ fontSize: 12, color: 'rgba(255,255,255,0.55)', lineHeight: 1.8 }}>{t(lang, descKo, descEn)}</p>
                </div>
              </div>
            ))}
          </div>
        ...
        <div>
          <div style={{ background: 'var(--panel)', border: '1px solid var(--border)', borderBottom: 'none', padding: '16px 28px', display: 'flex', alignItems: 'center', gap: 16 }}>
            ...
            <span style={{ fontFamily: "'Rajdhani', sans-serif", fontSize: 15, fontWeight: 700, color: 'var(--orange)', letterSpacing: 2, textTransform: 'uppercase' }}>{t(lang, '특허 현황', 'Patents')}</span>
            <span style={{ fontSize: 11, color: 'var(--muted)', fontFamily: "'DM Mono', monospace" }}>{t(lang, '자체 기술 개발 특허', 'Proprietary Technology Patents')}</span>
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 2, background: 'var(--border)' }}>
            {/* 특허 1 */}
            <div style={{ background: 'var(--card)', padding: '32px 28px', position: 'relative', overflow: 'hidden' }}>
              ...
              <div style={{ textAlign: 'center' }}>
                <div style={{ display: 'inline-block', padding: '3px 12px', background: 'rgba(245,124,0,0.1)', border: '1px solid rgba(245,124,0,0.3)', fontSize: 9, letterSpacing: 3, color: 'var(--orange)', fontFamily: "'DM Mono', monospace", textTransform: 'uppercase', marginBottom: 10 }}>{t(lang, '특허증 #1', 'Patent #1')}</div>
                <h4 style={{ fontFamily: "'Rajdhani', sans-serif", fontSize: 15, fontWeight: 700, color: 'var(--white)', marginBottom: 8, letterSpacing: 1 }}>
                  {t(lang, '전등제어 시스템 및 이를 위한 전등시위치연동 제어기', 'Lighting Switch Interlocking Controller')}
                </h4>
                <p style={{ fontSize: 11, color: 'var(--muted)', fontFamily: "'DM Mono', monospace" }}>{t(lang, '특허번호: 제 10-2232124 호', 'Patent No.: 10-2232124')}</p>
              </div>
            </div>
            ...
            <div style={{ background: 'var(--card)', padding: '32px 28px', position: 'relative', overflow: 'hidden' }}>
              <div style={{ position: 'absolute', top: 0, left: 0, right: 0, height: 2, background: 'linear-gradient(to right,var(--orange),transparent)' }}></div>
              <div style={{ border: '1px solid var(--border)', background: 'var(--panel)', overflow: 'hidden', marginBottom: 20, height: 200, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 10, background: 'rgba(245,124,0,0.04)' }}>
                ...
                <p style={{ fontSize: 10, color: 'var(--muted)', fontFamily: "'DM Mono', monospace", letterSpacing: 2, textAlign: 'center', textTransform: 'uppercase' }}>{t(lang, '이미지 준비중', 'Coming Soon')}</p>
              </div>
              <div style={{ textAlign: 'center' }}>
                <div style={{ display: 'inline-block', padding: '3px 12px', background: 'rgba(245,124,0,0.1)', border: '1px solid rgba(245,124,0,0.3)', fontSize: 9, letterSpacing: 3, color: 'var(--orange)', fontFamily: "'DM Mono', monospace", textTransform: 'uppercase', marginBottom: 10 }}>{t(lang, '특허증 #2', 'Patent #2')}</div>
                <h4 style={{ fontFamily: "'Rajdhani', sans-serif", fontSize: 15, fontWeight: 700, color: 'var(--white)', marginBottom: 8, letterSpacing: 1 }}>{t(lang, '특허명 준비중', 'Patent Title TBD')}</h4>
                <p style={{ fontSize: 11, color: 'var(--muted)', fontFamily: "'DM Mono', monospace" }}>{t(lang, '특허번호: 준비중', 'Patent No.: TBD')}</p>
              </div>
            </div>
            ...
            <div style={{ background: 'rgba(22,32,48,0.5)', padding: '32px 28px', border: '1px dashed rgba(245,124,0,0.15)', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 14, minHeight: 340 }}>
              ...
              <p style={{ fontSize: 10, color: 'rgba(255,255,255,0.2)', fontFamily: "'DM Mono', monospace", letterSpacing: 2, textAlign: 'center', textTransform: 'uppercase' }}>{t(lang, '특허 추가 예정', 'Patent To Be Added')}</p>
            </div>
          </div>
        </div>
      </div></div>

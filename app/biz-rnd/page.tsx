...
type RndItem = {
  ...
  titleKo: string
  titleEn: string
  feats: { ko: string; en: string }[]
  badges: { label: string; orange?: boolean }[]
}

const RND_ITEMS: RndItem[] = [
  {
    num: '01',
    titleKo: '생산관리 시스템',
...
export default async function BizRndPage() {
  ...
  return (
    <>
      <div className="bc">
        <div className="bc-in">
          <Link href="/">{t('common.home')}</Link>›<span>{t('rnd.bc')}</span>
        </div>
      </div>
      ...
      <div style={{ position: 'relative', height: 320, overflow: 'hidden' }}>
        ...
        <img
          src="/images/engineer-4.png"
          alt="R&D"
          style={{ width: '100%', height: '100%', objectFit: 'cover', objectPosition: 'center 30%' }}
        />
        <div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(to bottom,rgba(8,12,18,0.4),rgba(8,12,18,0.82))' }} />
        <div
          style={{
            position: 'absolute',
            inset: 0,
        ...
        >
          <div className="eyebrow" style={{ justifyContent: 'center', color: 'var(--orange)' }}>
          ...
          <p style={{ fontSize: 14, color: 'rgba(255,255,255,0.65)', marginTop: 10 }}>{t('rnd.hero.desc')}</p>
        </div>
      </div>
      ...
      <div className="outer">
        <div className="sec">
          <div className="vis-grid">
            {RND_ITEMS.map(({ num, titleKo, titleEn, feats, badges }) => (
              <div className="vis-card" key={num}>
                <div className="vis-num">{num}</div>
                <h3>{isKo ? titleKo : titleEn}</h3>
                ...
                <div className="tech-badges" style={{ marginTop: 16 }}>
                  {badges.map(({ label, orange }) => (
                    <span key={label} className="tech-badge" style={orange ? { borderColor: 'rgba(245,124,0,0.4)', color: 'var(--orange)' } : {}}>
                      {label}
                    </span>
                  ))}
                </div>
          ...
          <div style={{ marginTop: 70 }}>
            ...
            <h2 className="stitle">
              <span>{t('rnd.env.t1')}</span>
              <em>{t('rnd.env.t2')}</em>
            </h2>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gridTemplateRows: '260px 260px', gap: 2, background: 'var(--border)', marginTop: 36 }}>
              <div className="hz">
                {/* eslint-disable-next-line @next/next/no-img-element */}
                <img src="/images/rnd-office-1.png" alt="Office" />
              </div>
              <div className="hz">
                {/* eslint-disable-next-line @next/next/no-img-element */}
                <img src="/images/rnd-office-2.png" alt="Engineer" style={{ objectPosition: 'center top' }} />
              </div>
              <div className="hz" style={{ '--hz-scale': 1.03 } as React.CSSProperties}>
                {/* eslint-disable-next-line @next/next/no-img-element */}
                <img src="/images/rnd-part-1.png" alt="Workshop" />
              </div>
              <div className="hz">
                {/* eslint-disable-next-line @next/next/no-img-element */}
                <img src="/images/rnd-part-2.png" alt="Meeting" />
              </div>
            </div>
          </div>
        </div>
